...
def get_http_session() -> requests.Session:
    ...
    if _session is None:
        ...
        _session.headers.update({
            'User-Agent': 'MCP-Service-Template/1.0',
...
def get_country_info(
    ...
    try:
        ...
        search_urls = [
            f"{base_url}/name/{country}",
            f"{base_url}/alpha/{country}",
            f"{base_url}/capital/{country}"
        ]
        ...
        logger.error(f"HTTP request error: {str(e)}")
        return {
            "error": f"Failed to fetch country info: {str(e)}",
            "country": country
        }
    except Exception as e:
...
def get_placeholder_data(
    ...
    try:
        ...
        logger.error(f"HTTP request error: {str(e)}")
        return {
            "error": f"Failed to fetch placeholder data: {str(e)}",
            "resource": resource
        }
    except Exception as e:
