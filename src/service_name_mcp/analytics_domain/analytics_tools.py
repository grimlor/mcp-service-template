    ...
    try:
        ...
        mock_results = [
            ...
            },
            {
                ...
                "Data": [
                    ["Current Period Analysis", f"[🔗 Execute Query]({generate_query_link('SELECT * FROM current_period_data')})", "Current period analysis query"],
                    ["Comparison Period Analysis", f"[🔗 Execute Query]({generate_query_link('SELECT * FROM comparison_period_data')})", "Comparison period analysis query"]
                ],
...
@mcp.tool(
    description="Performs custom statistical analysis on your data with flexible query support."
)
def custom_analysis(
    analysis_type: str,
    data_query: str,
    parameters: Optional[str] = None,
    analysis_description: str = "Custom Analysis"
) -> List[Dict[str, Any]]:
    """
